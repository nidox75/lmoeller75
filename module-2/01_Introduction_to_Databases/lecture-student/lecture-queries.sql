...
SELECT DISTINCT census_region FROM state; -- Returns each unique row fro the row set
SELECT DISTINCT state_abbreviation FROM state;
...
SELECT city_name, area, population FROM city WHERE state_abbreviation IN ('CT' , 'ME', 'NH', 'RI', 'VT');
...
SELECT park_name, area * 1000 * 1000 AS area_in_meters FROM park;
...
SELECT park_name, park FROM park WHERE area BETWEEN 100 AND 200;
